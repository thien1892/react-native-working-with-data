...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // dayStreak.map((streak) => console.log(streak));
        
        console.log(JSON.parse(dayStreak));
        const dayStreakParse = String(JSON.parse(dayStreak));
        setDayStreak(dayStreakParse);
      } catch (e) {
  ...
  useEffect(
    () => {
      (async () => {
        try {
          await AsyncStorage.setItem('dayStreak', JSON.stringify(dayStreak));
        } catch (e) {
          Alert.alert(e.message);
        }
      })()
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.row}>
        ...
        <TextInput
          placeholder='Day streak'
          value={String(dayStreak)}
          onChangeText={(data) => { setDayStreak(data);}}
          />
      </View>
    </View>
  );
